...
router.post('/', writeLimiter, authenticate, async (req, res) => {
  try {
    ...
    if (!memberId) {
      res.status(403).json({ error: 'The authenticated account is not linked to a member.' });
      return;
    }

    if (requestedMemberId && requestedMemberId.toLowerCase() !== memberId.toLowerCase()) {
      res.status(403).json({ error: 'A member may only submit their own RSVP.' });
      return;
    }
